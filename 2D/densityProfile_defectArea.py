...
parser.add_argument("-o",
                    action="store", nargs='?', default="../Analysis/defect_area.dat",
                    required=False, dest="output",
                    help="output filename")
parser.add_argument("-i",
                    action="store", nargs='?', default="../Analysis/Output_scaler",
...
parser.add_argument("-r",
                    action="store", nargs='?', default=float('50'),
                    required=False, dest="bubble_r", type=float,
                    help="The radius of the bubble")
...
def compute_areas(box_v):

    boxArea = box_v[0]*box_v[1]
    if shape == "circle":
        bubbleArea = pi * np.power(bubble_r,2)
    elif shape == "cube":
        bubbleArea = 4 * np.power(bubble_r,2) * np.power(gamma(1.025),2) / gamma(1.05)
    elif shape == "squircle":
        bubbleArea = 4 * np.power(bubble_r,2) * np.power(gamma(1.25),2) / gamma(1.5)
    else:
        print sys.stderr,"Unknown shape, exiting ..."
        sys.exit(1)

    totalArea = boxArea - bubbleArea
...
def read_S(input_filename):
    ...
    return binsX, binsY, scalar, scalar_reshaped, density_reshaped, mesh
...
def compute_defect_area(binsX, binsY, scalar):
    deltaY = abs(binsY[1] - binsY[0])
    gridArea = np.power(deltaY, 2)
    counter = 0
    for i in range(len(scalar)):
        if scalar[i] < thresholdS:
            r = get_r(binsX[i], binsY[i])
            if r > bubble_r + 10:
                counter += 1
    return counter * gridArea

def write_defect_data(totalArea, boxArea, defectArea):
    outputFile = open(output_filename, 'w')
    outputFile.write("Box Area (including bubble) = " + str(boxArea) + '\n'
                     "Total Area (excluding bubble) = " + str(totalArea) + '\n'
                     "Defect Area = " + str(defectArea) + '\n'
                     "Percentage of Defect Areas = " + str((defectArea/totalArea)*100) + '\n')


# Calculate and plot the scalar order profiles
binsX, binsY, scalar, scalar_reshaped, density_reshaped, mesh = read_S(input_filename)
...
plot_data(r_sec_dia[:len(r_sec_dia)/2], scalar_sec_dia[:len(scalar_sec_dia)/2],
          ...
          outputname = '../Analysis/scalar_axis.png',
          ylabel_txt = r'$\mathrm{Scalar\/Order,\/S}$')

# Compute the area of defect regions
totalArea, boxArea = compute_areas(box_v)
defectArea = compute_defect_area(binsX, binsY, scalar)
write_defect_data(totalArea, boxArea, defectArea)
